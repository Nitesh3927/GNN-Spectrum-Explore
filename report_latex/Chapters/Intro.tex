...
% Deep learning has revolutionized many machine learning tasks in recent years, ranging from image classification and video processing to speech recognition and natural language understanding, where the data are typically represented in the Euclidean space \cite{comprehensive2021wu}. However, there is an increasing number of applications where data are generated from **non-Euclidean domains** and must be represented as graphs with complex relationships and interdependency between objects \cite{comprehensive2021wu}. Graphs are pervasive data structures used for modeling various real-world systems, including social networks, molecular structures in chemistry, traffic networks, and citation networks \cite{comprehensive2021wu}. Analyzing graph data is paramount for gaining insights into intricate patterns, uncovering hidden structures, and understanding the dynamics of interconnected systems \cite{realworld2025ju}.

% The complexity inherent in graph data poses significant challenges to conventional machine learning algorithms designed for regular Euclidean data \cite{comprehensive2021wu}. Unlike image data, which can be represented as a regular grid, a graph can be irregular, featuring a variable size of unordered nodes, and nodes may have a differing number of neighbors, resulting in operations (e.g., convolutions) being difficult to apply directly to the graph domain \cite{comprehensive2021wu}.

% In response to these challenges, **Graph Neural Networks (GNNs)** have emerged as a pivotal technology \cite{review2024khemani}, demonstrating remarkable success in handling complex relationships and intricate structural information inherent in graph-structured data \cite{methods2020zhou, review2024khemani}. GNNs are deep learning models designed to capture the dependence of graphs primarily via a **message-passing mechanism** between nodes \cite{methods2020zhou, review2024khemani}. The success of GNNs is partially motivated by extending the core ideas of deep neural architectures, such as Convolutional Neural Networks (CNNs), to generalize convolution operations to graph data \cite{comprehensive2021wu}. This field has since experienced significant growth, leading to the development of numerous powerful variants, including Graph Convolutional Networks (GCNs) and Graph Attention Networks (GATs) \cite{methods2020zhou, review2024khemani}.

% The rapid development and diversification of the GNN landscape necessitate a focused exploration and comparison of existing architectural approaches \cite{review2024khemani}. By systematically investigating different GNN architectures, the aim is to gain a holistic understanding of their foundational mechanisms, evaluate their performance trade-offs, and determine their suitability for various data characteristics and learning tasks.
